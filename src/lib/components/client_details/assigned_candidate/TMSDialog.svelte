<script lang="ts">
	...
	import type { AppliedCandidate } from '$lib/services/client_services/client.type';
	...
	const { job, showDialog, clientId } = $props<{
		job: AppliedCandidate;
		showDialog: boolean;
		candidateId: number;
		clientId: number;
	}>();

	let selectedUser: AppliedCandidate = $state(job); // Initialize with job prop
	...
	async function initializeUser() {
		...
		selectedUser = job;
		userTimesheets[job.candidate_id] = {
			userId: job.candidate_id,
			timeEntries:
				job.tsm?.map((entry: any) => ({
					...
					tsm_id: entry.tsm_id,
					status: entry.break_time_updated === 1 ? 1 : 2
				})) || []
		};
	}
	// tanstackquery
    ...
    try {
        ...
        if (success) {
            ...
            if (userTimesheets[selectedUser.candidate_id]) {
                const entries = userTimesheets[selectedUser.candidate_id].timeEntries;
                ...
                userTimesheets[selectedUser.candidate_id].timeEntries = updatedEntries;
            }
...
async function handleRejectEntry(entry: TimeEntry) {
	...
	function getFilteredEntries(entries: TimeEntry[]) {
		switch (selectedTab) {
			case 'awaiting':
				return entries.filter((e) => e.status === 1);
			case 'approved':
				...
				<div class="w-64 overflow-y-auto rounded-2xl bg-gray-50/50 p-4">
					...
					<div class="w-full rounded-xl bg-white p-3">
						<div class="flex items-center gap-3">
							...
							<span class="font-medium text-gray-700">{job.first_name} {job.last_name}</span>
						</div>
					</div>
					<div class="space-y-2">
						...
						<!-- <button
                            class="w-full p-3 rounded-xl text-left transition-all duration-200
                                {selectedUser.candidate_id === candidate.candidate_id ? 
                                'bg-white scale-[1.02]' : 
                            ...
                            <div class="flex items-center gap-3">
                                <div class="w-10 h-10 rounded-full bg-gray-100 flex items-center justify-center">
                                    {#if candidate.profile_picture}
                                        <img src={candidate.profile_picture} alt={`${candidate.first_name} ${candidate.last_name}`} class="w-full h-full rounded-full object-cover">
                                    {:else}
                                        <span class="material-icons-sharp text-gray-400">person</span>
                                    {/if}
                                </div>
                                <span class="font-medium text-gray-700">{candidate.first_name} {candidate.last_name}</span>
                            </div>
                        </button> -->
						<!-- {/each} -->
						<!-- {#each job.candidates as candidate}
				...
				<div class="w-72 overflow-y-auto rounded-2xl bg-gray-50/50 p-6">
					...
					<div class="space-y-6">
						...
						<div class="rounded-xl bg-white p-4">
							...
							<p class="mt-1 font-medium text-gray-800">{selectedUser.first_name}</p>
						</div>
						<div class="rounded-xl bg-white p-4">
						...
						<div class="rounded-xl bg-white p-4">
							...
							<p class="mt-1 font-medium text-gray-800">{selectedUser.location || 'N/A'}</p>
						</div>
					</div>
				</div>
				...
				<div class="flex flex-1 flex-col gap-6">
					...
					<div class="flex-1 rounded-2xl bg-gray-50/50 p-6">
						<div class="mb-6 flex items-center justify-between">
							...
							<div class="flex items-center gap-4 rounded-xl bg-gray-100 p-1">
								...
								>
									Awaiting ({userTimesheets[selectedUser.candidate_id]?.timeEntries.filter(
										(e) => e.status === 1
									).length || 0})
								</button>
								<button
								...
								>
									Approved ({userTimesheets[selectedUser.candidate_id]?.timeEntries.filter(
										(e) => e.status === 2
									).length || 0})
								</button>
								<button
								...
								>
									Rejected ({userTimesheets[selectedUser.candidate_id]?.timeEntries.filter(
										(e) => e.status === 3
									).length || 0})
								</button>
							</div>
						...
						<div class="max-h-[400px] space-y-4 overflow-y-auto pr-2">
							{#each getFilteredEntries(userTimesheets[selectedUser.candidate_id]?.timeEntries || []) as entry (entry.date)}
								<div
					...
					{#if selectedTab === 'approved'}
						{@const summary = calculatePaymentSummary(
							userTimesheets[selectedUser.candidate_id]?.timeEntries || []
						)}
						<div class="rounded-2xl bg-white p-6">
							<h3 class="mb-6 font-semibold text-gray-700">
								Payment Summary for {selectedUser.first_name}
